...
public class TC_MISC_AtleastOneBoxValueTest_006 extends base {
	LoginPage LoginPage;
...
//	newformexe.executeScript("arguments[0].click();", newform);
	...
	public void SaveAndContinue() throws InterruptedException{
		...
		Thread.sleep(30000);
		keyF8();
		Log.info("Key F8 pressed ");
		Thread.sleep(50000);
		WebElement VESpage=getDriver().findElement(By.xpath("//h3[contains(text(),'Select Forms')]"));
		boolean vesDisplay=VESpage.isDisplayed();
		if(vesDisplay==true) {
			
			Log.info("VES page launched ");
		}else {Log.info("VES page not launched. SO, testcase failed");}
