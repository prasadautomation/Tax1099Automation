...
public class TC_AddRecipient_010 extends base {
	...
	@Test(priority=2)
	public void AddrecipientEinNoTIN_nonUs() throws InterruptedException {
...
WebElement businessname=getDriver().findElement(By.id("PayeeLastName"));
		...
		WebElement EINtextbox=getDriver().findElement(By.id("PayeeFedaralID"));
		...
		if (checkboxenabled==false) {
			...
			boolean EINenabled=EINtextbox.isEnabled();
			Log.info("Is EIN text box is enabled after clicked on TIN not provided:"+EINenabled);
			if (EINenabled==false) {
				Log.info("EIN text box is disabled after clicked on TIN not provided");
			}
